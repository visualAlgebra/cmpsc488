...
http3.onreadystatechange = function () {
	if ( http3.readyState == 4 && http3.status == 200) {
		console.log("new lesson: " + http3.response);
	}
}
http3.open("GET", "http://localhost:8080/lessons/NEW_LESSON", true);
http3.setRequestHeader("Content-type", "application/json");
http3.setRequestHeader('account', 'TEST_ACCOUNT');
http3.send();
...
http4.onreadystatechange = function () {
	if ( http4.readyState == 4 && http4.status == 200) {
		console.log("new problem: " + http4.response);
	}
}
http4.open("GET", "http://localhost:8080/problems/NEW_PROBLEM", true);
...
http5.onreadystatechange = function () {
	if ( http5.readyState == 4 && http5.status == 200) {
		console.log("new account: " + http5.response);
	}
}
http5.open("GET", "http://localhost:8080/accounts/TEST_ACCOUNT", true);
